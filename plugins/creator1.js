...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
 ...
 let ftroli = { key: { remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { itemCount: 9999999999, status: 1, surface: 1, message: `${command} ${type}`, orderTitle: wm, sellerJid: '0@s.whatsapp.net' } } }
  let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Zero MD⸙;;;\nFN:Zero MD⸙\nORG:Zero MD⸙\nTITLE:\nitem1.TEL;waid=6289529129992:+62 895-2912-9992\nitem1.X-ABLabel:Zero MD⸙\nX-WA-BIZ-DESCRIPTION:${htjava} Ingfokan mabar terraria!!\nX-WA-BIZ-NAME:Zero MD⸙\nEND:VCARD`
//------- NOMOR
...
{title: "⸙ Biodata Zero MD", rowId: ".owner bio"},
{title: "⸙ Biodata Jovan", rowId: ".owner bio2"},
{title: "⸙ Pengembang", rowId: ".zeromods"},
{title: "⸙ Creator", rowId: ".zeromods1"},
...
await conn.sendMessage(m.chat, { contacts: { displayName: wm, contacts: [{ vcard }] }}, { quoted: ftroli })
 ...
 conn.send2ButtonImg(m.chat, fs.readFileSync('./media/menu.jpg').buffer, teksnomor, `Halo ${tag}\nItu cuy\n` + botdate, `Sewa Bot`, `.sewa`, 'Menu', `.menu`, ftroli, { contextInfo: { externalAdReply: { showAdAttribution: true,
    ...
    mediaType: 2, 
    description: sgc,
    title: "Kaga usah join w kaga punya gc",
    body: wm,
   ...
   conn.sendButton(m.chat, '┍┈༺ *BioData ZeroMD* ༻', teksbio, ppown, [
          ...
          conn.sendButton(m.chat, '┍┈༺ *Kaga Ada* ༻', teksbio2, ppown2, [
                ['Sewa', `${usedPrefix}sewa`],
                ['Menu', `${usedPrefix}menu`]
            ], m)
            break
